...
define(["require", "exports", "N/email", "N/error", "N/file", "N/format", "N/log", "N/record", "N/runtime", "N/search", "N/url"], function (require, exports, email, error, file, format, log, record, runtime, search, url) {
    ...
    function generateItems(savedSearchId, priceLevel) {
        let itemRows = '';
        let headerRow = '<tr>';
        ...
        const columnNames = [];
        const searchColumns = itemSearch.columns;
        for (const column of searchColumns) {
            columnNames.push(column.name);
        }
        if (!~columnNames.indexOf(priceFieldId))
            itemSearch.columns = searchColumns.concat([search.createColumn({ name: priceFieldId, label: 'Price' })]);
 ...
 `;
    ...
    function checkExistingCustomerRecord(parameters, configSettings, paymentInstruments) {
        if (!parameters['custEmail'])
            return '';
        let customerId;
        ...
        if (existingCustomerSearch.length > 0) {
            ...
            if (!paymentInstruments)
                ensureCustomerHasLegacyCreditCard(customerId, parameters); // Add the card if it isn't there
        }
            ...
            if (paymentInstruments)
                addNewPaymentInstrumentRecord(customerId, parameters); // Adds payment card for new customer if payment instruments is on
        }
        return customerId;
    }
    /** Adds address to the customer record **/
    function addCustomerAddress(customer, parameters) {
    ...
    function checkAddressExists(customerId, parameters) {
    ...
    function ensureCustomerHasLegacyCreditCard(customerId, parameters) {
        let creditCardNo = '';
        ...
        for (let line = 0; line < existingCustomer.getLineCount({ sublistId: 'creditcards' }); line++) {
            const longCardNo = existingCustomer.getSublistValue({ sublistId: 'creditcards', fieldId: 'ccnumber', line });
            ...
            expiryDateRecord = existingCustomer.getSublistValue({ sublistId: 'creditcards', fieldId: 'ccexpiredate', line });
    ...
    function getPaymentInstrument(customerId, parameters) {
        const cardNo = parameters['cardNo'];
        const inputtedFourDigits = cardNo.substring(cardNo.length - 4); // Last four digits of card entered on suitelet
    ...
    function newCustomer(subsidiaryId, accountId, parameters, paymentInstruments) {
        ...
        try {
            ...
            if (!paymentInstruments)
                addLegacyCreditCard(customer, parameters); // this one selects a line... might not work that way on this record
    ...
    function createTransactionRecord(customerId, parameters, config) {
        if (!customerId)
            return 'No customer - please ensure an email address was entered.'; // Someone managed to do a blank POST on 27 Jan 2022
        ...
        if (createdTransactionId) {
            ...
            if (transactionType == 'Sales Order') {
                ...
                if (paymentAuthorised || total == 0) {
                    ...
                    const transactionNumber = transactionValues['tranid'];
                    const paymentTotal = transactionValues['fxamount'];
                    const currencies = transactionValues['currency'];
    ...
    function sendEmailNotification(config, recordId) {
        if (!config.emailAuthorId || !config.emailNotificationAddr)
            return;
        let recordURL = '';
        if (config.transactionType == 'Cash Sale')
            recordURL = url.resolveRecord({ recordType: 'cashsale', recordId });
        if (config.transactionType == 'Sales Order')
            recordURL = url.resolveRecord({ recordType: 'salesorder', recordId });
        let body = `Created ${config.transactionType || 'transaction'} ID ${recordId} successfully.`;
        if (recordURL)
            body += ` Click <a href="${recordURL}">here</a> to see the transaction in NetSuite.`;
    ...
    function setPaymentCard(parameters, customerId, transaction, itemLocation, paymentProcessingProfile) {
    ...
    function checkPaymentAuthorisation(createdTransactionId) {
        let authorizationResult = '', authorizationReason = '', captureResult = '', captureReason = '', amount = '', paymentAccepted;
        search.create({
            type: 'transaction',
            ...
            const eventType = result.getValue({ name: 'paymenteventtype', join: 'paymentEvent' });
            const eventStatus = result.getValue({ name: 'paymentstatus', join: 'paymentEvent' });
            const reason = result.getValue({ name: 'holdreason', join: 'paymentEvent' });
            amount = result.getValue({ name: 'amount', join: 'paymentEvent' });
    ...
    function addNewPaymentInstrumentRecord(customerId, parameters) {
        try {
    ...
    function addLegacyCreditCard(customer, parameters) {
    ...
    function transactionSuccessfulPage(companyLogo, createdTransactionId, transactionNumber, paymentTotal, currencyId) {
  ...
  `;
    ...
    function transactionFailedPage(companyLogo) {
  ...
  `;
    ...
    function salesOrderAccepted(companyLogo) {
  ...
  `;
    ...
    function formatDateInSearch(date) {
    ...
    function configurationSettings(parameters, customerId) {
        let filters;
        let companyLogo = '';
        if (!customerId) {
            ...
            const subsidiary = customerSearch['subsidiary'];
        ...
        if (results.length > 0) {
            ...
            const paymentProcessingProfile = configRecord.getValue('custrecord_hitc_pay_processing_file_ess');
            const subsidiaryId = configRecord.getValue('custrecord_hitc_subsidiary_ess');
            const companyName = configRecord.getValue('custrecord_hitc_name_ess');
            let headerAddress = configRecord.getValue('custrecord_hitc_address_ess');
            headerAddress = headerAddress.replace(/\n/g, '<br>');
            const accountId = configRecord.getValue('custrecord_hitc_account_ess');
            const accountName = configRecord.getText('custrecord_hitc_account_ess');
            const savedSearchId = configRecord.getValue('custrecord_hitc_saved_search_ess');
            const itemLocation = configRecord.getValue('custrecord_hitc_location_ess');
            const emailNotificationAddr = configRecord.getValue('custrecord_hitc_notify_email_addr_ess');
            const emailAuthorId = configRecord.getValue('custrecord_hitc_notify_email_author_ess');
            const priceLevel = configRecord.getValue('custrecord_hitc_price_level_ess');
            const transactionType = configRecord.getText('custrecord_hitc_tran_type_ess');
            const htmlTemplate = configRecord.getValue('custrecord_hitc_html_template_ess');
            const companyLogoChoice = configRecord.getValue('custrecord_hitc_company_logo_ess');
            if (companyLogoChoice) {
                const logoField = search.lookupFields({ type: 'file', id: companyLogoChoice, columns: ['url'] });
                companyLogo = logoField["url"];
            }
